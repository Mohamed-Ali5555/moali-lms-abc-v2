...
class OfflinePaymentController extends Controller
{
    public function store(Request $request)
    {
        ...
        // remove from cart
        if ($request->item_type == 'course' ||$request->item_type == 'book' || $request->item_type == 'mixed') {
            $url = 'purchase.history';
            CartItem::whereIn('course_id', $item_id_arr)->where('user_id', auth()->user()->id)->where('type','course')->delete();
            CartItem::whereIn('book_id', $item_id_arr)->where('user_id', auth()->user()->id)->where('type','book')->delete();
