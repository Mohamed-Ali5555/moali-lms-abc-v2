...
<div class="quiz-wizard lesson-wizard tf-modal-form">
    <div class="lesson-wizard__banner">
        <div>
            ...
            <p class="lesson-wizard__desc">{{ get_phrase('Set title, category, duration and scoring rules') }}</p>
        </div>
        <div class="lesson-wizard__course">
            <span>{{ get_phrase('Status') }}</span>
            <strong id="quizStatusBadge">{{ get_phrase('Active') }}</strong>
        </div>
    </div>

    <form action="{{ route('admin.bank.quizs.store') }}" method="post">
        @csrf
        ...
        <div class="lesson-form-section">
            ...
            <div class="fpb7 mb-3">
                ...
                <input class="form-control ol-form-control" type="text" id="title" name="title"
                    value="{{ old('title') }}" placeholder="{{ get_phrase('Enter quiz title') }}" required>
            </div>
                        ...
                        @if ($category->bank_category->count())
                            <optgroup label="{{ $category->title }}">
                                @foreach ($category->bank_category as $sub_category)
                                    <option value="{{ $sub_category->id }}" @selected(old('category') == $sub_category->id)>
                                        {{ $sub_category->title }}
                                    </option>
                                @endforeach
                            </optgroup>
            ...
            <div class="mb-0">
                ...
                <div class="tf-choice tf-choice--2">
                    <label for="quiz_status_1">
                        <input type="radio" name="status" id="quiz_status_1" value="1" @checked(old('status', '1') == '1')
                    ...
                    <label for="quiz_status_0">
                        <input type="radio" name="status" id="quiz_status_0" value="0" @checked(old('status') === '0')
            ...
            <div class="quiz-metric-grid">
                <div>
                    ...
                    <input class="form-control ol-form-control" type="number" min="0" max="23"
                        id="hour" name="hour" value="{{ old('hour', 0) }}" placeholder="0">
                </div>
                <div>
                    ...
                    <input class="form-control ol-form-control" type="number" min="0" max="59"
                        id="minute" name="minute" value="{{ old('minute', 30) }}" placeholder="30">
                </div>
                <div>
                    <label class="form-label ol-form-label" for="second">{{ get_phrase('Second') }}</label>
                    <input class="form-control ol-form-control" type="number" min="0" max="59"
                        id="second" name="second" value="{{ old('second', 0) }}" placeholder="0">
                </div>
            </div>
            ...
            <div class="quiz-metric-grid">
                <div>
                    ...
                    <input class="form-control ol-form-control" type="number" min="1"
                        id="total_mark" name="total_mark" value="{{ old('total_mark', 10) }}" required>
                </div>
                <div>
                    ...
                    <input class="form-control ol-form-control" type="number" min="1"
                        id="pass_mark" name="pass_mark" value="{{ old('pass_mark', 5) }}" required>
                </div>
                <div>
                    ...
                    <input class="form-control ol-form-control" type="number" min="1"
                        id="retake" name="retake" value="{{ old('retake', 1) }}" required>
                </div>
            </div>
        ...
        <div class="lesson-form-section">
            ...
            <div class="mb-0">
                <textarea name="description" rows="4" class="form-control ol-form-control"
                    placeholder="{{ get_phrase('Optional description for students') }}">{{ old('description') }}</textarea>
