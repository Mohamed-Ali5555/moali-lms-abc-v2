            ...
            <div class="book-form__preview" id="thumbnailPreview">
                ...
                <div class="d-flex gap-4 flex-wrap">
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="file_type" id="file_type_file" value="file"
                            {{ ($book->file_type ?? 'file') === 'file' ? 'checked' : '' }}>
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="file_type" id="file_type_link" value="link"
                            {{ ($book->file_type ?? '') === 'link' ? 'checked' : '' }}>
                        <label class="form-check-label" for="file_type_link">{{ get_phrase('External link') }}</label>
                    </div>
                </div>
                </div>
   <div class="mb-3" id="book_file_wrap">
                <label for="book_file" class="form-label ol-form-label">{{ get_phrase('PDF file') }}</label>
                <input type="file" name="book_file" class="form-control ol-form-control" id="book_file"
                    accept="application/pdf,.pdf" />
                @if (!empty($book->file_path))
                    <small class="text-success d-block mt-1">
                        {{ get_phrase('Current file') }}:
                        <a href="{{ asset($book->file_path) }}" target="_blank">{{ basename($book->file_path) }}</a>
                    </small>
                @endif
                <small class="text-muted">{{ get_phrase('Leave empty to keep current file') }}</small>
            </div>
            ...
            <div class="mb-3" id="book_link_wrap" style="display:none;">
                ...
                <input type="url" name="file_url" class="form-control ol-form-control" id="file_url"
                    placeholder="https://..." value="{{ $book->file_url }}" />
