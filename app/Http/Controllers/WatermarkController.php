...
class WatermarkController extends Controller
{
    ...
    public static function makeTempWatermark($logo)
    {
        ...
        if (! File::exists($temp_path)) {
            File::makeDirectory($temp_path, 0777, true);
        }
        ...
        $watermark_full_path = public_path($watermark);
        
        // Ensure parent directory exists
        $watermark_dir = dirname($watermark_full_path);
        if (!is_dir($watermark_dir)) {
            File::makeDirectory($watermark_dir, 0777, true);
        }
