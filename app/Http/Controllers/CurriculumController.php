...
class CurriculumController extends Controller
{
    ...
    public function lesson_store(Request $request)
    {
        ...
        Lesson::insert($data);
        $lesson = Lesson::where('course_id', $data['course_id'])
            ->where('title', $data['title'])
            ->orderByDesc('id')
            ->first();
        if ($lesson) {
            app(WhatsAppNotifier::class)->notifyLessonPublished($lesson);
        }
    ...
    /**
     * Copy or move lesson to another course
     */
    public function lesson_copy_or_move(Request $request)
    {
        ...
        $lesson = Lesson::with(['course', 'section', 'questions'])->findOrFail($request->lesson_id);
        $targetSection = Section::where('course_id', $request->target_course_id)
            ->findOrFail($request->target_section_id);

        if ($lesson->course_id == $request->target_course_id) {
        ...
        try {
            if ($request->action == 'copy') {
                ...
                $newLesson->course_id = $request->target_course_id;
                $newLesson->section_id = $request->target_section_id;
                $newLesson->sort = $maximum_sort_value + 1;
                $newLesson->save();

                if ($lesson->lesson_type == 'quiz' && $lesson->questions->isNotEmpty()) {
                    foreach ($lesson->questions as $question) {
                        $newQuestion = $question->replicate();
                        $newQuestion->quiz_id = $newLesson->id;
                        $newQuestion->save();
                    }
                }

                Session::flash('success', get_phrase('Lesson copied successfully.'));
            } else {
                $lesson->course_id = $request->target_course_id;
                $lesson->section_id = $request->target_section_id;
                $maxLesson = Lesson::where('course_id', $request->target_course_id)
                    ->orderBy('sort', 'desc')
                    ->first();
                $lesson->sort = ($maxLesson ? $maxLesson->sort : 0) + 1;
                $lesson->save();

                Session::flash('success', get_phrase('Lesson moved successfully.'));
            }
