...
class BookController extends Controller
{
    public function index(Request $request, $category = '')
    {
        ...
        if (request()->has('search')) {
            $query->where(function ($query) {
                $query->where('books.title', 'LIKE', '%' . request()->input('search') . '%');
                $query->orWhere('books.short_description', 'LIKE', '%' . request()->input('search') . '%');
                $query->orWhere('books.level', 'LIKE', '%' . request()->input('search') . '%');
                $query->orWhere('books.meta_keywords', 'LIKE', '%' . request()->input('search') . '%');
                $query->orWhere('books.disc', 'LIKE', '%' . request()->input('search') . '%');
            });
        }
        ...
        $page_data['books']  = $query->latest('id')->paginate($layout == 'grid' ? 9 : 5)->appends($request->query());
    ...
    public function book_details(Request $request, $slug)
    {
        ...
        if ($book->exists()) {
            ...
            $page_data['book_details'] = $book_details;
            // $page_data['sections']       = Section::where('book_id', $book_details->id)->orderBy('sort')->get();
            // $page_data['enroll']         = Enrollment::where('course_id', $book_details->id)->count('user_id');

            $view_path = 'frontend.' . get_frontend_settings('theme') . '.book.book_details';
