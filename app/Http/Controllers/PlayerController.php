        ...
        if ($course->is_paid && auth()->user()->role != 'admin') {
            if (auth()->user()->role == 'student') { // for student check enrollment status
                ...
                if ($enroll_status == 'expired') {
                    Session::flash('error', get_phrase('انتهت صلاحية وصولك إلى الدورة التدريبية. عليك شراؤها مرة أخرى.'));
                    return redirect()->back();
                } elseif ($enroll_status == 'free_locked') {
                    Session::flash('error', get_phrase('هذا الكورس أصبح مدفوعاً. عليك شراؤه للوصول إلى المحتوى.'));
                    return redirect()->back();
