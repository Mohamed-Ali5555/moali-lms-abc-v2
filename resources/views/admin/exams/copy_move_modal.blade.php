@php
    $exam = \App\Models\Lesson::with('course', 'section')->find($id);
    $courses = \App\Models\Course::with('category.parent')->orderBy('title')->get();
@endphp
@if ($exam && $exam->lesson_type == 'quiz' && $exam->type == 1)
@php
    $otherCourses = $courses->where('id', '!=', $exam->course_id);
@endphp
<form action="{{ route('admin.exams.copy.or.move') }}" method="post" id="exam-copy-move-form">
    @csrf
    <input type="hidden" name="exam_id" value="{{ $exam->id }}">

    <div class="fpb7 mb-3">
        <label class="form-label ol-form-label">{{ get_phrase('Exam') }}</label>
        <input class="form-control ol-form-control" type="text" value="{{ $exam->title }}" readonly>
        <small class="text-muted">{{ get_phrase('Course') }}: {{ $exam->course->title ?? 'N/A' }}</small>
    </div>
    ...
    <div class="fpb7 mb-3">
        ...
        <small class="text-muted d-block mt-1">{{ get_phrase('Copy') }}: {{ get_phrase('Keep exam in current course and add copy to target course') }}</small>
        <small class="text-muted d-block">{{ get_phrase('Move') }}: {{ get_phrase('Remove exam from current course and add to target course') }}</small>
    </div>
...
$(function() {
    ...
    $('#target_course_id').on('change', function() {
        ...
        $.get('{{ url("admin/exams/load-sections") }}/' + courseId, function(data) {
            $('#no-sections-msg').addClass('d-none');
    ...
    $('#exam-copy-move-form').on('submit', function() {
...
@include('admin.init')
@else
    <p class="text-danger mb-0">{{ get_phrase('Exam not found.') }}</p>
@endif
