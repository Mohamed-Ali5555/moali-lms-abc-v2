                        ...
                        <div class="row mt-4">
                            <div class="col-sm-6">
                                <div class="">
                                    ...
                                    <h3 class="eh5 mt-2">#{{ $report_history->invoice }}</h3>
                                </div>

                                <h6 class="title fs-18px mt-4">{{ get_phrase('Billed To') }}</h6>
                                <address>
                                    {{ get_user_info($report_history->user_id)->name }},
                                    {{ get_user_info($report_history->user_id)->email }}
                                </address>
                            </div> <!-- end col-->
                            ...
                            <div class="col-auto ms-auto">
                                <div class="d-flex justify-content-between">
                                    <div class="mt-3">
                                        <img width="200px" src="{{ asset(get_frontend_settings('dark_logo')) }}"
                                            alt="">

                                        <div class="ps-3 mt-4">
                                            <h6 class="title fs-12px">{{ get_phrase('Issue Date') }}</h6>
                                            <address>
                                                {{ date('d-M-Y', strtotime($report_history->created_at)) }}
                                            </address>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th>{{ get_phrase('Price') }}</th>
                                                   <th>{{ get_phrase('سعر المنتج') }}</th>

                                                <th class="text-right">{{ get_phrase('Amount') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach ($report_invoices as $report)
                                                <tr>
                                                    <td class="min-w-250px">
                                                        <h4 class="title fs-14px">{{ $report->item->title ?? '' }}</h4>
                                                    ...
                                                    <td width="110px">
                                                        @if ($report->item->discount_price > 0)
                                                            <p>{{ currency($report->item->discount_price, 2) }}</p>
                                                        @else
                                                            <p>{{ currency($report->item->price, 2) }}</p>
                                                        @endif
                                                    </td>
                                                      ...
                                                      <td width="110px">
                                                        @if ($report->qty > 0)
                                                            <p>{{ currency($report->amount / $report->qty, 2) }}</p>
                                                        @endif
                                                    </td>
                                            ...
                                            <tr>
                                                ...
                                                <td class="min-w-250px">
                                                    <p>
                                                        <span>{{ currency($report_invoices->sum('amount')) }}</span>
                                                    </p>
                                                    <p>
                                                        <span>{{ currency($report_history->tax) }}</span>
                                                    </p>

                                                    <p>
                                                        <span>{{ currency($report_invoices->sum('amount') - $report_history->tax) }}</span>
                                                    </p>
                                                </td>
                                            </tr>
                                        </tbody>
