...
<div class="mb-3">
    <h6 class="mb-1">{{ get_phrase('Basic information') }}</h6>
    <small class="text-muted">{{ get_phrase('Title, description, category, and visibility.') }}</small>
</div>
...
<div class="row mb-3">
    <label for="show_on_home" class="form-label ol-form-label col-sm-2 col-form-label">
        {{ get_phrase('الصفحة الرئيسية') }}
    </label>
    <div class="col-sm-10">
        <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" role="switch" id="show_on_home"
                name="show_on_home" value="1"
                @checked((int) ($course_details->show_on_home ?? 0) === 1)>
            <label class="form-check-label" for="show_on_home">
                {{ get_phrase('إظهار في أحدث الكورسات بالصفحة الرئيسية') }}
            </label>
        </div>
        <small class="text-muted d-block mt-1">
            {{ get_phrase('يظهر الكورس في سايدبار أحدث الكورسات في الرئيسية') }}
        </small>
    </div>
</div>
