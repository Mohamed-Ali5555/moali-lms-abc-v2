<script>
    ...
    window.ensureCkEditor = function(callback) {
        function ready() {
            if (typeof callback === 'function') callback();
        }

        function waitUntilReady(retries) {
        ...
        var src = '{{ asset('/ckeditor/ckeditor.js') }}';
        if (window.lmsLoadAsset) {
            lmsLoadAsset.js(src).catch(function() {});
    ...
    window.ensureTagify = function(callback) {
        function ready() {
            if (typeof callback === 'function') callback();
        }
        ...
        function waitUntilReady(retries) {
            ...
            if (retries <= 0) {
                console.error('Tagify failed to load');
                return;
            }
            setTimeout(function() {
                waitUntilReady(retries - 1);
        ...
        var cssHref = '{{ asset('assets/global/tagify-master/dist/tagify.css') }}';
        var jsSrc = '{{ asset('assets/global/tagify-master/dist/tagify.min.js') }}';

        if (window.lmsLoadAsset) {
            Promise.all([
                lmsLoadAsset.css(cssHref),
                lmsLoadAsset.js(jsSrc)
            ]).catch(function() {});
        } else {
    ...
    window.initializeDurationPickers = function(fields) {
        var src = '{{ asset('assets/global/duration-picker/DurationPickerMaker.js') }}';
        var run = function() {
            if (typeof window.__lmsInitDurationPickers === 'function') {
                window.__lmsInitDurationPickers(fields);
                return;
            }
            fields.forEach(function(field) {
                var pickerElement = document.querySelector(field);
                if (!pickerElement || typeof DurationPickerMaker === 'undefined') {
                    return;
                }
                var formattedDuration = new FormattedDuration({
                    hoursUnitString: ":",
                    minutesUnitString: ":",
                    secondsUnitString: "",
                });
                var durationPickerMaker = new DurationPickerMaker(formattedDuration);
                durationPickerMaker.SetPickerElement(pickerElement, window, document);
            });
        };
        if (typeof DurationPickerMaker !== 'undefined') {
            run();
            return;
        }
        if (window.lmsLoadAsset) {
            lmsLoadAsset.js(src).then(run);
        } else {
            var script = document.createElement('script');
            script.src = src;
            script.onload = run;
    ...
    function downloadPDF(elem = ".print-table", fileName = 'data') {
        ...
        if (typeof html2pdf === 'undefined') {
            var src = '{{ asset('assets/backend/js/html2pdf.bundle.min.js') }}';
            if (window.lmsLoadAsset) {
                lmsLoadAsset.js(src).then(run);
            } else {
                var script = document.createElement('script');
