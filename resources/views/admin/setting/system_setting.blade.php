        ...
        <div class="admin-toolbar">
            <div class="admin-toolbar__meta">
                ...
                <div>
                    ...
                    <p class="admin-toolbar__desc">{{ get_phrase('Configure system name, branding, and defaults') }}</p>
                </div>
            </div>
        </div>
                ...
                <div class="ol-card-body">
                    <div class="col-lg-12">
                        ...
                        <form class="required-form" action="{{ route('admin.system.settings.update') }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="fpb-7 mb-3">
                                <label class="form-label ol-form-label" for="quiz_submission_mode">{{ get_phrase('Quiz/Assignment submission mode') }}</label>
                                <select class="form-control ol-form-control ol-select2" data-toggle="select2" name="quiz_submission_mode" id="quiz_submission_mode">
                                    <option value="normal" @if (get_settings('quiz_submission_mode') != 'secure') selected @endif>{{ get_phrase('Normal Mode') }}</option>
                                    <option value="secure" @if (get_settings('quiz_submission_mode') == 'secure') selected @endif>{{ get_phrase('Exam Mode (Strict)') }}</option>
                                </select>
                                <small class="text-muted">
                                    <strong>{{ get_phrase('Normal Mode') }}:</strong> الإجابات تُحفظ فقط عند الضغط على تسليم أو انتهاء الوقت. الطالب يقدر يعمل refresh ويبدأ من جديد.<br>
                                    <strong>{{ get_phrase('Exam Mode') }}:</strong> كل إجابة تُحفظ وتُصحح فوراً. لو الطالب خرج أو عمل refresh، يحصل على درجة الأسئلة المُجابة فقط.
                                </small>
                            </div>
