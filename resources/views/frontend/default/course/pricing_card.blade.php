    ...
    <div class="ps-box ps-sidebar">
        ...
        <div class="ps-price d-flex">
            ...
            @elseif (isset($course_details->discount_flag) && $course_details->discount_flag == 1)
                <h4 class="g-title">
                    {{ currency($course_details->discount_price, 2) }}</h4>
            ...
            @else
                ...
                @else
                    ...
                    @if (isset($course_details->is_paid) && $course_details->is_paid == 1)
                        @if ($in_cart)
                            <a href="{{ route('cart.delete', ['id' => $course_details->id, 'type'=>'course']) }}" class="eBtn mt-3 gradient w-100">
                                {{ get_phrase('Remove from cart') }}</a>
                        @else
                            <a href="{{ route('cart.store', ['id' => $course_details->id, 'type' => 'course']) }}"  class="eBtn learn-btn w-100 mb-3 mt-3">
