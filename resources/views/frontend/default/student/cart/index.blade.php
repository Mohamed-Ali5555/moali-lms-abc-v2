            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                    <div class="entry_panel">
                        <h4 class="g-title text-20 mb-4">{{ get_phrase('Cart items') }}</h4>
                        ...
                        @if(count($cart_items) > 0 || count($cart_items_books) > 0 )
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-6 mb-30 pb-4">
                                            <div class="row">
                                                <div class="col-lg-3 col-md-3">
                                                    <img class="radius-10 object-fit-cover w-100" height="160px"
                                                        src="{{ get_image($course->thumbnail) }}" alt="course-thumbnail">
                                                </div>
                                                                ...
                                                                <h4>
                                                                    ...
                                                                    @else
                                                                        @if ($course->discount_flag == 1)
                                                                            ...
                                                                            <b
                                                                                class="text-dark">{{ currency($discount_price) }}</b>
                                                                            <del
                                                                                class="text-12px">{{ currency($course->price, 2) }}</del>
                                                                        @else
                                                                            @php
                                                                                $count_items_price += $course->price;
                                                                            @endphp
                                                                            <b
                                                                                class="text-dark">{{ currency($course->price, 2) }}</b>
                                                                        @endif
                                                                    @endif
                                                                </h4>
                                                            </div>
                                                            <div class="learn-more">
                                                                <a data-bs-toggle="tooltip" title="{{ get_phrase('Delete') }}"
                                                                    href="{{ route('cart.delete', ['id' => $course->id , 'type'=>'course']) }}">
                            ...
                            <div class="col-12 py-5 text-center">
                                <h4 class="g-title">{{ get_phrase('No courses found !') }}</h4>
                            </div>
                            @endif
                            {{-- /////////////// --}}
                            {{-- /////////////// --}}
                            @if (count($cart_items_books) > 0)
                                <div class="row">
                                    @foreach ($cart_items_books as $book)
                                        <div class="col-lg-12 col-md-12 col-sm-6 mb-30 pb-4">
                                            <div class="row">
                                                <div class="col-lg-3 col-md-3">
                                                    <img class="radius-10 object-fit-cover w-100" height="160px"
                                                        src="{{ get_image($book->thumbnail) }}" alt="course-thumbnail">
                                                </div>
                                                <div class="col-lg-9 col-md-9">
                                                    <div class="entry-details">
                                                        <div class="entry-title en-title">
                                                            <h3 class="ellipsis-2">{{ ucfirst($book->title) }}</h3>
                                                        </div>
                                                        <p class="description ellipsis-2">{!! ellipsis(strip_tags($book->disc), 200) !!}</p>
                                                        <div class="learn-creator">
                                                            <div class="creator">
                                                                ...
                                                                <h4>
                                                                    ...
                                                                    @endphp
                                                                                                                                        <b class="text-dark">{{ currency($book->price, 2) }}</b> price
                                                                                                                                        <b class="text-dark">{{  $book->qty }}</b> qty

                                                                    <b class="text-dark">{{ currency($book->price * $book->qty, 2) }}</b> total
                                                                </h4>
                                                            </div>
                                                            <div class="learn-more">
                                                                <a data-bs-toggle="tooltip" title="{{ get_phrase('Delete') }}"
                            ...
                            @else
                            <div class="col-12 py-5 text-center">
                                <h4 class="g-title">{{ get_phrase('No books found !') }}</h4>
                            </div>
                    ...
                    <div class="entry_panel">
                        <div class="cart-total-price px-0 px-lg-4">
                            ...
                            @php
                                ...
                                $tax =
                                    (get_settings('course_selling_tax') / 100) *
                                    ($count_items_price - $coupon_discount);
                            @endphp
                            ...
                            <form action="{{ route('payout') }}" method="post" class="mt-20">@csrf
                                        ...
                                        <div class="col-md-12">
                                            @if (request()->has('coupon') && isset($coupon) && $coupon_discount > 0)
                                                <div class="alert w-100 alert-purple show d-flex align-items-center py-2">
                                                    <div>
                                                        {{ get_phrase('Coupon') }} <strong>{{ get_phrase('Applyed') }}
                                                            ({{ $coupon->discount }}%) !</strong>
                                                    </div>
                                                    <a href="{{ route('cart') }}" type="button"
                                                        class="btn ms-auto mt-2"><i
                                        ...
                                        <div class="col-md-12">
                                            <div class="input-group mb-3">
                                                <input type="text" class="form-control" name="coupon"
                                                    placeholder="{{ get_phrase('Apply coupon') }}"
                                                    value="{{ request()->query('coupon') }}">
                                                <button type="button" value="{{ get_phrase('Apply') }}"
                                                    class="input-group-text eBtn gradient text-white" id="apply-coupon">
                                                    {{ get_phrase('Apply') }}
                                                </button>
                                            </div>
                                        </div>
                                ...
                                <div class="mt-20 send_gift_check">
                                    <div class="form-check">
                                        <input class="form-check-input mt-0" type="checkbox" name="is_gift"
                                            value="1" id="send_gift">
                                        <label class="form-check-label"
                                            for="send_gift">{{ get_phrase('Send as a gift') }}</label>
                                    </div>

                                    <input type="email" class="form-control mt-15 gifted_user d-none" name=""
                                        placeholder="{{ get_phrase('Enter user email') }}">
                                </div>

                                <div class="mt-20">
                                    <input type="submit" class="form-control eBtn gradient text-white"
                                        value="{{ get_phrase('Continue to payment') }}"
