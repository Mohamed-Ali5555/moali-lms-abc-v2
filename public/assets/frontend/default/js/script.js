$(document).ready(function () {
    ...
    document.querySelectorAll(".userDropDown li").forEach((li) => {
    ...
    if (savedTheme) {
        // إذا كان هناك ثيم محفوظ في localStorage، نستخدمه
        applyTheme(savedTheme);
        toggleMode.checked = savedTheme === "dark";
    } else {
        // إذا لم يكن هناك ثيم محفوظ، نتحقق من media query
        updateThemeFromMediaQuery();
    }

    // تغيير الثيم عند تغيير media query (فقط إذا لم يكن هناك قيمة محفوظة)
    darkModeMediaQuery.addEventListener("change", updateThemeFromMediaQuery);

    // تغيير الثيم يدويًا وتخزين الحالة في localStorage
    ...
    function applyTheme(theme) {
        if (theme === "dark") {
            ...
            $(".header-dark-logo").removeClass("d-none");
            $(".header-light-logo").addClass("d-none");
        } else {
            document.body.classList.remove("dark-mode");
            $(".header-dark-logo").addClass("d-none");
            $(".header-light-logo").removeClass("d-none");
        }
    }

    function updateThemeFromMediaQuery() {
        const theme = darkModeMediaQuery.matches ? "dark" : "light";
        applyTheme(theme);
        toggleMode.checked = theme === "dark";
    }
